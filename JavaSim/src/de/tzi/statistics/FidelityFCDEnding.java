 ...
 *
 */
public class FidelityFCDEnding extends AbstractStatistics {
	
	public FidelityFCDEnding(TrafficManager trafficManager) {
	...
	public boolean update(int time) {
		...
		for (PassingPossibility pp : pps) {
			int measureTime_t = propertyManager.getPassingPossibilityProperty(
							PassingPossibilityProperty.FCD_ENDING_UPDATE, pp.getId());
			...
			if (measureTime_t > 0 && measureTime_all > 0) {
				int t_t = propertyManager.getPassingPossibilityProperty(PassingPossibilityProperty.FCD_ENDING_TIME, pp.getId());
	...
	public String getName() {
		return "Fidelity of FCD readouts at crossings";
	}
}
